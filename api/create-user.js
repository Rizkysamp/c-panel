export default async function handler(req, res) {
  ...
  const panelUrl = "https://lenzzofficial.serverku.biz.id"; // Ganti dengan panel kamu
  const apikey = "ptla_kKep5NCdEHYv5qaF0ff1i2zg1Bo7VXJLzuaPubwMPzx"; // Ganti API key
  ...
  try {
    ...
    if (serverRes.status !== 201) {
      return res.status(500).json({ error: 'Gagal buat server', detail: serverData });
    }

    return res.status(200).json({
      message: '✅ Server berhasil dibuat!',
      user: { email, username, password, id: userId },
      server: {
